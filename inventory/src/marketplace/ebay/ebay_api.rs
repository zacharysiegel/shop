...
use crate::marketplace::ebay::ebay_client::{AuthorizationCodeResponse, ClientCredentialsResponse};
...
async fn get_application_token() -> impl Responder {
    let token_response: ClientCredentialsResponse = unwrap_result_else_500!(
        super::ebay_client::get_application_token().await
    );
...
async fn get_user_token(
    ...
    let user_token_response: AuthorizationCodeResponse = unwrap_result_else_500!(
        super::ebay_client::get_user_token(authorization_code).await
    );
