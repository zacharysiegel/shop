...
async fn get_listing(pgpool: web::Data<PgPool>, id: web::Path<String>) -> impl Responder {
    let id = unwrap_result_else_400!(Uuid::try_parse(id.into_inner().as_str()));
    let listing = unwrap_result_else_500!(listing_db::get_listing(&pgpool, &id).await);
