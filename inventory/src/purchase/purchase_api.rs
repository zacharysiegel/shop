...
pub fn configurer(config: &mut ServiceConfig) {
    config.service(
        web::scope("/purchase")
            ...
            .route(
                "/{purchase_id}/listing",
                web::get().to(get_purchase_listing),
            ),
    );
}
...
async fn create_purchase(
    ...
    let query_result =
        unwrap_result_else_500!(purchase_db::create_purchase(&pgpool, &purchase.to_entity()).await);

    HttpResponse::Ok().body(query_result.rows_affected().to_string())
}

async fn get_purchase(pgpool: web::Data<PgPool>, purchase_id: web::Path<String>) -> impl Responder {
    ...
    purchase.to_serial().to_http_response()
}
...
async fn get_purchase_listing(
    ...
    let listing =
        unwrap_result_else_500!(purchase_db::get_purchase_listing(&pgpool, &purchase_id).await);
